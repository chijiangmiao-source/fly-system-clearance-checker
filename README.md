...
运行（各段等时，或按选填的 `duration_weights` 分配各段相对耗时），精确求两矩形的全程首次接触。
...
- **禁入区启用窗口**：可在 `zones[]` 项中选填 `active_window`，以全程百万分之一刻度的
  起止整数（`0 ≤ start_tick ≤ end_tick ≤ 1000000`）描述该区的启用区间；省略时视为全程生效。
  各段接触区间与启用窗口换算到同一精确时间轴后，仅在闭区间重叠（含端点相触）处计碰撞，
  最早碰撞时刻为 max(首触时刻, 窗口起点)；页面在禁入区旁标注生效区间，
  仅穿过未启用区域的路线仍显示全绿。
...
```json
{
  ...
  "zones": [
    { "id": "A", "vertices": [ {"x": 4000, "y": 0}, {"x": 6000, "y": 0},
      ...
      "active_window": { "start_tick": 250000, "end_tick": 750000 } }
  ]
}
```
...
- `zones[].active_window` 选填：须为含 `start_tick` / `end_tick` 整数的对象，按禁入区顺序
  检查 `0 ≤ start_tick ≤ end_tick ≤ 1000000`（全程百万分之一刻度），省略时该区全程生效；
  格式或顺序错误定位到 `zones.<i>.active_window` 下的具体字段
  （如 `zones.0.active_window.end_tick`）。
...
```json
{
  ...
  "active_window_display": { "start": "0.600000", "end": "0.800000" }
}
```
...
- `active_window`：命中禁入区填写的启用窗口（`start_tick` / `end_tick` 原样回显）；
  该区未填写窗口时为 `null`；
- `active_window_display`：窗口起止刻度换算到 t 轴的展示值（half-up 六位，如
  `{"start": "0.600000", "end": "0.800000"}`）。

**单段兼容**：不含 `waypoints`（或 `waypoints: []`）时 `segment_index` 恒为 `0`，
`segment_t_display` 与 `t_display` 相同，`t`、`position` 等原字段含义与数值完全不变。
未填写 `active_window` 的旧请求检测结果不变（窗口字段为 `null`）。
...
在**同一总时长** t∈[0,1] 内运行（段内匀速）。默认各段等时；每套吊景可选填
`duration_weights`（正整数数组，长度 = 段数）表示各路线段的相对耗时，折点时刻取累计权重
归一化的精确有理数。服务端按双方折点时刻的并集切分共同时间轴，用**精确有理数**（`Fraction`）
计算两个轴对齐矩形的首次接触；**边界接触也算冲突**。输入仍沿用台口尺寸与折线坐标规则，但无 `zones`。
...
- `duration_weights` 选填：须为正整数数组且长度等于该吊景路线段数（停位数 + 1），
  表示各段相对耗时；长度不符定位到 `fly_a.duration_weights` / `fly_b.duration_weights`，
  非整数或非正整数定位到具体下标（如 `fly_a.duration_weights.1`）；省略时各段等时，
  旧方案无需修改即得原有结果；
- 双方分段数可以不同。
...
（各自安全段/危险段、起终与停位姿态）、双方首次接触姿态与接触点；提供 `duration_weights`
的吊景会在其路线各段旁标出相对耗时与占总时长比例，检测结果仍展示双方命中段、段内时刻与
全程时刻；校验失败时保留本次原文并清除上一次结果。权重按**原文逐位精确**展示，任意长度的
正整数（含十六位以上乃至三百位）都不会被浮点舍入、溢出为 `Infinity` 或导致标注缺失，
权重总和同样按十进制精确累加展示（前端以保留超大整数字面量的严格解析读取原文，不经 `Number`）。
...
- 启用窗口：矩形与单条多边形边的接触时刻集是单个闭区间（凸形相对平移），
  由各顶点-边事件接触子区间的首尾端点合成（t=0 / t=1 的跨穿与纯包含由静态判定补足）；
  禁入区的接触时刻集是各边区间的**并集**（非凸区可互不相交，如 U 形空腔两侧的臂），
  再加上无边界接触空隙中的纯包含时段——空腔等不接触间隙不会被并入。
  各接触区间与窗口（按段换算到同一时间轴）均为闭区间，仅在重叠处计碰撞，
  最早碰撞时刻为各重叠段 max(区间起点, 窗口起点) 的最小值，责任边按该时刻重新评估。

双吊景交会（`app/encounter.py`）不涉及多边形：按双方折点时刻并集切分共同时间轴（等时为
`i/n`、`j/m`；加权时为累计权重之比的精确有理数折点），小区间内双方各处于固定分段，两矩形
在 x、y 轴上的边界坐标均为 t 的仿射函数。每个轴向“两移动区间相交”的时刻集是一个闭区间
（相对位置 −s_b ≤ d(t) ≤ s_a，等号即接触），两轴区间相交即发生接触，取交集左端为该小区间
首次接触时刻；自左向右扫描、严格更小时替换（折点接触归前段）。全程 `Fraction` 运算，
并用 1200 组随机路线（含随机权重）与精确有理数稠密采样交叉核对。
...
- `verify`：一次性服务，等待 API 健康后执行碰撞/无碰撞/缺字段/自交/超长整数/
  ...
  双吊景交会（起始即接触、分段数不同中途相撞、时间错开安全、第二套越界、编号相同、
  各段相对耗时——加权错开为安全、加权改换命中段、权重长度/非正校验、原接口不回归）/
  禁入区启用窗口（先穿过未启用区再命中启用区的折线、窗口端点接触算碰撞、
  窗口错开则相同几何命中安全、窗口推迟碰撞时刻、未填窗口旧样例不回归、
  窗口格式/顺序/越界与按区顺序的字段定位、U 形空腔只启用空腔时段不误报）/
  Web 页面断言，全部通过退出码 0。
...
python -m pytest tests/ -q                 # 用例：临界值、责任决胜、校验路径、折线分段、双吊景交会（含各段相对耗时）、禁入区启用窗口
