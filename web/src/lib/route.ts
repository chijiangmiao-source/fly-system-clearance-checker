import type { CheckResult, StagePayload, StagePoint } from './types';

/** 完整折线的顶点：start → 各中途停位 → end。 */
export function routePoints(payload: StagePayload): StagePoint[] {
  return [payload.fly.start, ...(payload.fly.waypoints ?? []), payload.fly.end];
}
...
export function splitRouteAtHit(payload: StagePayload, result: CheckResult): RouteSplit | null {
  ...
  const points = routePoints(payload);
  const segmentIndex = result.segment_index;
  if (segmentIndex < 0 || segmentIndex >= points.length - 1) {
    return null;
  }
  const hit = result.position;
  // 命中点恰在折点上（段内 t=0/1）时，命中点与相邻折点重合，折叠重复点
  ...
  const safe = [...safeTail, ...(safeTail.length === 0 || !same(safeTail[safeTail.length - 1], hit) ? [hit] : [])];
  const danger = [...(dangerHead.length === 0 || !same(dangerHead[0], hit) ? [hit] : []), ...dangerHead];
