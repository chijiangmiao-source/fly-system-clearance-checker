...
t∈[0,1] 内运行：默认各分段等时、段内匀速；每套吊景可选填 duration_weights
（正整数数组，长度 = 段数）表示各路线段的相对耗时，此时折点时刻为累计权重
归一化的精确有理数。判定两矩形全程是否接触（边界接触也算冲突），并求全程
最早冲突时刻及当时双方所在段号、双方姿态与接触位置。

共同时间轴按双方折点时刻（等时为 i/n、j/m；加权时为累计权重之比）的并集
切分；每个小区间内双方各自位于固定分段，矩形在 x、y 两个轴向上的边界坐标
都是 t 的仿射函数。两个轴对齐闭矩形相交当且仅当两个移动区间在 x、y 轴上
同时相交：每个轴的相交时刻集是一个闭区间（相对位置为仿射函数），两轴区间
相交即发生接触，取交集左端为该小区间的首次接触时刻。全程使用 Fraction
精确求解。
"""
...
def cumulative_breaks(nseg: int, weights: Optional[Sequence[int]] = None) -> List[Fraction]:
    """单套吊景的折点时刻严格递增序列（含 0、1）。

    未提供 duration_weights 时各段等时（i/nseg）；否则第 i 个折点为
    前 i 段权重之和 / 权重总和——精确有理数。
    """
    if not weights:
        return [Fraction(i, nseg) for i in range(nseg + 1)]
    total = sum(weights)
    marks = [Fraction(0)]
    acc = 0
    for w in weights:
        acc += w
        marks.append(Fraction(acc, total))
    return marks


def _axis_motion(
    points: Sequence[Point], seg: int, breaks: Sequence[Fraction], axis: int
) -> Tuple[Fraction, Fraction]:
    """该吊景位于第 seg 段时，指定轴（0=x,1=y）左下角坐标关于 t 的仿射系数。

    第 seg 段的时间窗为 [breaks[seg], breaks[seg+1]]，段内参数
    u = (t − breaks[seg]) / (breaks[seg+1] − breaks[seg])，
    左下角坐标 = p_seg + u·(p_seg+1 − p_seg) = v·t + c，
    返回 (v, c) = (Δ/段时长, p_seg − breaks[seg]·v)。
    """
    t0, t1 = breaks[seg], breaks[seg + 1]
    delta = points[seg + 1][axis] - points[seg][axis]
    v = Fraction(delta) / (t1 - t0)
    c = points[seg][axis] - t0 * v
    return v, c


def _axis_overlap_window(
    va: Fraction, ca: Fraction, sa: int, vb: Fraction, cb: Fraction, sb: int,
...
def encounter_first_contact(fly_a: dict, fly_b: dict):
    ...
    折点两侧同时命中（前段段内 t=1 与后段段内 t=0 等价）归前段：小区间自左向右
    ...
    # 各自折点时刻：省略 duration_weights 时各段等时，否则按累计权重归一化
    ta = cumulative_breaks(len(pa) - 1, fly_a.get("duration_weights"))
    tb = cumulative_breaks(len(pb) - 1, fly_b.get("duration_weights"))
    marks = sorted(set(ta) | set(tb))
    ...
    for k in range(len(marks) - 1):
        ...
        ia = bisect_right(ta, t0) - 1
        ib = bisect_right(tb, t0) - 1

        avx, acx = _axis_motion(pa, ia, ta, 0)
        avy, acy = _axis_motion(pa, ia, ta, 1)
        bvx, bcx = _axis_motion(pb, ib, tb, 0)
        bvy, bcy = _axis_motion(pb, ib, tb, 1)
    ...
    t, ia, ib = best
    # 双方姿态（左下角）与段内时刻
    ua = (t - ta[ia]) / (ta[ia + 1] - ta[ia])
    ub = (t - tb[ib]) / (tb[ib + 1] - tb[ib])
